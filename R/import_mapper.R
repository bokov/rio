...
#' @export
import_mapper <- function(file, format, setclass, which, ...) {
    if (grepl("^http.*://", file)) {
    ...
    if (fmt %in% c(names(extmap),names(getOption('extmap.local')))) {
        if (missing(which)) { 
            x <- extmapper(fmt, file = file, ...)
        } else {
            x <- extmapper(fmt, file = file, which = which, ...)
        }
    } else {
        warning("Falling over to .import methods")
        if (missing(which)) {
            x <- .import(file = file, ...)
        } else {
